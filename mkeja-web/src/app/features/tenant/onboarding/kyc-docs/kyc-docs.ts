...
@Component({
  ...
  templateUrl: './kyc-docs.html',
  styleUrls: ['./kyc-docs.css']
})
export class KycDocs implements OnInit, OnDestroy {
  invitationCode = '';
  tenantId = '';
  ...
  // Flag to determine if this is onboarding/registration flow
  ...
  ngOnInit() {
    ...
    // Check if this is a registration flow (has invitation code or coming from registration)
    this.isRegistrationFlow = !!this.invitationCode || 
                             this.route.snapshot.queryParams['registration'] === 'true' ||
                             this.route.snapshot.queryParams['onboarding'] === 'true';

    if (queryTenantId) {
      this.tenantId = queryTenantId;
      sessionStorage.setItem('tenantId', queryTenantId);
      this.onboardingFlow = this.isRegistrationFlow;
      // Only load existing documents if this is NOT a registration flow
    ...
    if (this.authService.isAuthenticated() && !this.isRegistrationFlow) {
      this.resolveTenantContext();
      return;
    }

    this.tenantId = sessionStorage.getItem('tenantId') || '';
    this.onboardingFlow = this.isRegistrationFlow || !!this.tenantId;

    if (!this.tenantId) {
      this.contextError = 'Start from your invitation link or sign in to upload documents.';
    } else if (!this.isRegistrationFlow) {
      // Only load existing docs if not in registration flow
      this.loadExistingDocuments();
    }
  }
  ...
  get hasExistingDocuments(): boolean {
    // In registration flow, we don't show existing documents
    return this.isRegistrationFlow ? false : this.existingDocs.length > 0;
  }

  get isUpdateMode(): boolean {
    // In registration flow, it's never update mode
    return this.isRegistrationFlow ? false : this.hasExistingDocuments;
  }
  ...
  get pageSubtitle(): string {
    ...
    return 'We need to verify your identity before you can complete registration.';
  }
  ...
  slotHasExisting(slot: DocSlot): boolean {
    // In registration flow, we don't show existing files
    return this.isRegistrationFlow ? false : !!this.existingPreviews[slot];
  }
  ...
  private resolveTenantContext() {
    ...
    this.tenantService.getMyOnboardingContext().subscribe({
      next: (context) => {
        ...
        this.onboardingFlow = this.isRegistrationFlow || this.authService.isAuthenticated();
        this.resolvingContext = false;
        // Only load existing documents if not in registration flow
        if (!this.isRegistrationFlow) {
          this.loadExistingDocuments();
        }
  ...
  private loadExistingDocuments() {
    // Don't load existing documents during registration flow
    if (this.isRegistrationFlow) {
      return;
    }
  ...
  private loadExistingPreviews() {
    // Don't load existing previews during registration flow
    if (this.isRegistrationFlow) {
      return;
    }
  ...
  onSubmit() {
    ...
    // Prevent updates for existing users during registration flow
    if (this.isRegistrationFlow && this.authService.isAuthenticated() && this.hasExistingDocuments) {
      this.showError('This flow is for new registrations only. Please use the update flow for existing documents.');
      return;
    }
    ...
    const submitUpload = (tenantId: string) => {
      ...
      this.tenantService.uploadKycDocuments(tenantId, {
        ...
        next: (response) => {
          ...
          // For registration flow, always navigate to pending
          if (this.isRegistrationFlow || this.onboardingFlow) {
